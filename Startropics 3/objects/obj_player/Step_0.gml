if(!attack_flag) {
	...
	if(keyboard_check(global.kb_right) || gamepad_button_check(0, global.ctrl_right) || (gamepad_axis_value(0, gp_axislh) > 0.1)) {
		move_anim = 0;
		move_flag = true;
	...
	} else if(keyboard_check(global.kb_left) || gamepad_button_check(0, global.ctrl_left) || (gamepad_axis_value(0, gp_axislh) < -0.1)) {
		move_anim = 3;
		move_flag = true;
		x -= move_speed * delta_time/global.micro
	...
	if(keyboard_check(global.kb_up) || gamepad_button_check(0, global.ctrl_up) || (gamepad_axis_value(0, gp_axislv) < -0.1)) {
		if(move_flag) { 
			move_anim += 1;
		} else {
			move_anim = 6;
	...
	} else if(keyboard_check(global.kb_down) || gamepad_button_check(0, global.ctrl_down) || (gamepad_axis_value(0, gp_axislv) > 0.1)) {
		if(move_flag) {
			move_anim += 2;
		} else {
			move_anim = 7;
	...
	if(keyboard_key_press(global.kb_shoot) || gamepad_button_check_pressed(0, global.ctrl_shoot) || mouse_check_button_pressed(mb_left)) {
		attack_flag = true;
		switch(move_anim) {
			case 0:
				attack_angle = 0;
...
if(!jump_flag && (keyboard_key_press(global.kb_jump) || gamepad_button_check_pressed(0, global.ctrl_jump))) {
	jump_flag = true;
	jump_time = 0;
	move_speed = 100;
	elevation++;
